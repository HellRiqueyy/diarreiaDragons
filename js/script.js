document.addEventListener('DOMContentLoaded', function() {
  ...
  if (btnTriagem) {
    btnTriagem.addEventListener('click', () => {
      ...
      const respostaDiv = document.getElementById('respostaAI');
      respostaDiv.textContent = 'Analisando...';

      if (!sintomas.trim()) {
        respostaDiv.textContent = 'Por favor, insira os sintomas.';
        return;
      }
      ...
      if (especialidadeEncontrada) {
        respostaDiv.textContent = `Especialidade médica recomendada: ${especialidadeEncontrada}.`;
      } else {
        respostaDiv.textContent = 'Não foi possível identificar uma especialidade médica específica. Recomendamos procurar um médico para avaliação detalhada.';
      }
    });
  }
});
